...
class NewRecordPresenter constructor(
                        ...
                        recordRepository.newRecord(request!!)
                            ...
                            .switchMap<PartialChange> { createRecordResponse ->
                                if (request!!.validators.isNotEmpty()) {
                                    validatorsRepository.requestValidations(
                                        createRecordResponse.id,
                                        request!!.validators.map { it.id })
                                        .subscribeOn(Schedulers.io())
                                        .observeOn(AndroidSchedulers.mainThread())
                                        .map {
                                            NewRecordPartialChanges.CreateRecordEnd(
    ...
    override fun stateReducer(
        vs: LRViewState<NewRecordModel>,
        change: PartialChange
    ): LRViewState<NewRecordModel> {
        var result: LRViewState<NewRecordModel>? = null
        if (change !is NewRecordPartialChanges)
            result = super.stateReducer(vs, change)

        when (change) {
            is NewRecordPartialChanges.CreateRecordEnd -> result = vs.copy(
                closeScreen = true,
                model = vs.model.copy(sendingCreateRecord = false, sendingCreateRecordError = null)
            )

            is NewRecordPartialChanges.CreateRecordStart -> result = vs.copy(
                model = vs.model.copy(
                    sendingCreateRecord = true,
                    sendingCreateRecordError = null
                )
            )

            is NewRecordPartialChanges.CreateRecordError -> result = vs.copy(
                model = vs.model.copy(
                    sendingCreateRecord = false,
                    sendingCreateRecordError = change.error
                )
            )

            is NewRecordPartialChanges.SelectCategory -> result =
                vs.copy(model = vs.model.copy(item = vs.model.item.copy(category = change.category)))

            is NewRecordPartialChanges.SelectType -> result =
                vs.copy(model = vs.model.copy(item = vs.model.item.copy(recordType = change.type)))

            is NewRecordPartialChanges.SetValue -> result =
                vs.copy(model = vs.model.copy(item = vs.model.item.copy(value = change.value)))

            is NewRecordPartialChanges.SelectValidator -> result =
                vs.copy(model = vs.model.copy(item = vs.model.item.selectValidator(change.validator)))

            is NewRecordPartialChanges.PreviousStep -> result =
                vs.copy(model = vs.model.copy(currentStep = vs.model.currentStep - (if (vs.model.currentStep < NewRecordView.NUM_PAGES && vs.model.currentStep > 0) 1 else 0)))

            is NewRecordPartialChanges.NextStep -> result =
                vs.copy(model = vs.model.copy(currentStep = vs.model.currentStep + (if (vs.model.currentStep < NewRecordView.NUM_PAGES - 1 && vs.model.currentStep >= 0) 1 else 0)))
        }
